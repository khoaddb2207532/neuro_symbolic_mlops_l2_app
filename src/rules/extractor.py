...
class RuleExtractor:
    def extract(self, rf_model) -> RuleSet:
        all_rules: List[Rule] = []
        for tree_id, tree in enumerate(rf_model.estimators_):
            self._traverse_node(tree.tree_, 0, [], all_rules, tree_id)
        return RuleSet(rules=all_rules)

    def _traverse_node(self, tree_struct, node_id: int, current_conditions, all_rules, tree_id: int = -1) -> None:
        ...
        if left == _tree.TREE_LEAF or right == _tree.TREE_LEAF:
            ...
            all_rules.append(Rule(current_conditions.copy(), target_class,
                                  tree_id=tree_id, leaf_node_id=node_id))
            return
        feat, thres = tree_struct.feature[node_id], tree_struct.threshold[node_id]
        current_conditions.append(Condition(feat, "<=", thres))
        self._traverse_node(tree_struct, left, current_conditions, all_rules, tree_id)
        current_conditions.pop()
        current_conditions.append(Condition(feat, ">", thres))
        self._traverse_node(tree_struct, right, current_conditions, all_rules, tree_id)
        current_conditions.pop()
